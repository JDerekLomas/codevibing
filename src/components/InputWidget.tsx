...
export default function InputWidget({
  ...
  return (
    <div id="input-widget" className="fixed bottom-4 right-4 z-[9999] font-sans">
      {showToast && (
      ...
      {showOnboarding && mode === "idle" && !activeElement && (
        <div className="absolute bottom-14 right-0 w-64 rounded-lg bg-white p-3 shadow-xl border border-gray-200">
          <div className="flex items-start gap-2 mb-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600 mt-1">
                {isShareMode ? (
                  <>Leave your feedback using <strong>Edit</strong> or <strong>Comment</strong>. When done, click <strong>Copy feedback</strong> and paste it back.</>
                ) : (
          ...
          <button onClick={dismissOnboarding} className="w-full mt-2 text-xs text-gray-400 hover:text-gray-600 py-1">
            Got it
          </button>
      ...
      {mode === "idle" && !activeElement && (
        <div className="flex flex-col items-end gap-2">
          <div className="flex gap-2">
            <button
              onClick={() => { handleFirstAction(); setMode("editing"); }}
              className={`flex items-center gap-2 rounded-full bg-amber-600 px-4 py-2 text-sm font-medium text-white shadow-lg hover:bg-amber-500 ${showOnboarding ? "animate-pulse ring-2 ring-amber-400 ring-offset-2" : ""}`}
            ...
            <button
              onClick={() => { handleFirstAction(); setMode("commenting"); }}
              className={`flex items-center gap-2 rounded-full bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-lg hover:bg-blue-500 ${showOnboarding ? "animate-pulse" : ""}`}
            ...
            {isShareMode ? (
              <button
                ...
                className={`flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium shadow-lg transition ${inputCount > 0 ? "bg-green-600 text-white hover:bg-green-500 animate-pulse ring-2 ring-green-400 ring-offset-2" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
          ...
          {isShareMode && (
            <a href="https://getinput.io" target="_blank" rel="noopener noreferrer" className="text-[10px] text-gray-400 hover:text-gray-600 transition">
              Powered by getinput
            </a>
          )}
        </div>
      ...
      {(mode === "editing" || mode === "commenting") && !activeElement && (
        <div className="rounded-lg bg-white p-4 shadow-xl border border-gray-200">
          <p className="mb-2 text-sm text-gray-900">
            {mode === "editing" ? "Click any text to edit it directly" : "Click any element to leave a comment"}
          </p>
      ...
      {mode === "commenting" && activeElement && (
        <div className="w-72 rounded-lg bg-white p-4 shadow-xl border border-gray-200">
          <p className="mb-2 truncate text-xs text-gray-400">
            {activeElement.textContent?.slice(0, 50) || getSelector(activeElement)}
          </p>
          <textarea
            ref={commentInputRef}
            value={comment}
          ...
          <div className="flex justify-end gap-2">
            <button onClick={cancelComment} className="rounded px-3 py-1.5 text-sm text-gray-400 hover:text-gray-600">Cancel</button>
            <button onClick={submitComment} disabled={!comment.trim()} className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-500 disabled:opacity-50">Save</button>
          </div>
        </div>
      )}
      ...
      {mode === "viewing" && (
        <div className="w-80 rounded-lg bg-white p-4 shadow-xl border border-gray-200">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-gray-900">Feedback ({feedbackItems.length})</h3>
          ...
          {feedbackItems.length === 0 ? (
            <p className="text-sm text-gray-400 py-4 text-center">
              {isShareMode ? "Leave some feedback, then copy and send it back." : "No feedback yet. Use Edit or Comment to add some."}
            </p>
          ) : (
            <>
              ...
              {isShareMode ? (
                <>
                  <button onClick={copyFeedback} className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg py-2.5 text-sm font-medium transition">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <rect x="9" y="9" width="13" height="13" rx="2" ry="2" />
                      <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" />
                    </svg>
                  ...
                  <p className="text-[10px] text-gray-400 text-center mt-2">Paste this into Slack, email, or wherever you communicate</p>
                </>
              ) : (
                <>
                  <div className="flex gap-2">
                    <button onClick={copyFeedback} className="flex-1 flex items-center justify-center gap-2 bg-gray-900 hover:bg-gray-800 text-white rounded-lg py-2 text-sm font-medium transition">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        <rect x="9" y="9" width="13" height="13" rx="2" ry="2" />
                        <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" />
                      </svg>
                    ...
                    <button onClick={shareReviewUrl} className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg py-2 text-sm font-medium transition">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <p className="text-[10px] text-gray-400 text-center mt-2">Copy JSON for Claude Code, or share link with others</p>
                </>
              )}
            </>
          )}
